...
export const FloatingParticle = ({ delay = 0, index = 0, performanceLevel }: FloatingParticleProps) => {
  ...
  const sizes = sizeVariants[performanceLevel]
  ...
  // Enhanced particle properties with more variety
  const getParticleVariants = () => {
    const baseVariant = {
      high: {
        y: [-40 - (index % 3) * 10, -80 - (index % 4) * 15, -40 - (index % 3) * 10],
        x: [-15 + (index % 5) * 6, 15 - (index % 5) * 6, -15 + (index % 5) * 6],
        opacity: [0, 0.8 + (index % 3) * 0.1, 0],
        scale: [0.3 + (index % 3) * 0.2, 1.2 + (index % 4) * 0.3, 0.3 + (index % 3) * 0.2],
        rotate: [0, 180 + (index % 6) * 30, 360],
      },
      medium: {
        y: [-25 - (index % 3) * 8, -50 - (index % 3) * 10, -25 - (index % 3) * 8],
        x: [-8 + (index % 3) * 4, 8 - (index % 3) * 4, -8 + (index % 3) * 4],
        opacity: [0, 0.6 + (index % 2) * 0.1, 0],
        scale: [0.5 + (index % 2) * 0.2, 1.1 + (index % 3) * 0.2, 0.5 + (index % 2) * 0.2],
        rotate: [0, 90 + (index % 4) * 45, 180],
      },
      low: {
        y: [-20 - (index % 2) * 5, -35 - (index % 2) * 8, -20 - (index % 2) * 5],
        x: [-5 + (index % 2) * 3, 5 - (index % 2) * 3, -5 + (index % 2) * 3],
        opacity: [0, 0.4 + (index % 2) * 0.1, 0],
        scale: [0.7, 1.0 + (index % 2) * 0.1, 0.7],
      },
    }
    return baseVariant[performanceLevel]
  }
  ...
  const getParticleBackground = () => {
    ...
    const gradientSet = gradients[performanceLevel]
  ...
  return (
    <motion.div
      className={`absolute ${sizeClass} ${sizeClass.replace('w-', 'h-')} rounded-full`}
      style={{
        background: getParticleBackground(),
        left: `${baseX}%`,
        top: `${baseY}%`,
        willChange: "transform, opacity",
        filter: performanceLevel === "high" ? `blur(${0.3 + (index % 3) * 0.2}px)` : "none",
        boxShadow: getParticleShadow(),
      }}
      animate={getParticleVariants()}
      transition={{
        duration: performanceLevel === "high" 
